# KUBERNETES SECRETS TLS

## Features
- uses the type `kubernetes.io/tls`
- base64 encoded

## Example
remember to always base64 encode the key / crt

imperative with kubectl:
```bash
# handles base64 encoding for you
kubectl -n myns create secret tls admission-webhook-tls \
  --cert "admission-webhook-server-tls.crt" \
  --key "admission-webhook-server-tls.key"
```

declarative:
```yaml
apiVersion: v1
kind: Secret
metadata:
  name: secret-tls
type: kubernetes.io/tls
data:
  # values are base64 encoded, which obscures them but does NOT provide
  # any useful level of confidentiality
  tls.crt: |
    
    RklDQVRFLS0tLS0K    
  # In this example, the key data is not a real PEM-encoded private key
  tls.key: |
    RXhhbXBsZSBkYXRhIGZvciB0aGUgVExTIGNydCBmaWVsZA==
```